...
async function extractHeaderLabels(
  ...
  const bucketOf = (w: PositionedWord) => {
  ...
  const cols: PositionedWord[][] = Array.from({ length: boundaries.length + 1 }, () => []);
...
async function recognizeRowWords(worker: Awaited<ReturnType<typeof import("tesseract.js").createWorker>>, cropped: Buffer): Promise<PositionedWord[]> {
...
async function ocrTableBlock(
  ...
  const standardPass: PositionedWord[][] = [];
