...
# reuse the same OpenRouter key the app's Chat AI feature reads (see .env), if present
os.environ.setdefault("OPENROUTER_API_KEY", os.environ.get("OPENROUTER_KEY", ""))

# =============================================================================
# 1. CONFIGURATION
# =============================================================================
# Input files and the output folder are passed as arguments (never hardcoded
# here) — same convention as scripts/daily_sales_performance.py.

_parser = argparse.ArgumentParser(description="Daily Sales Performance — Executive Report")
...
_parser.add_argument("--output", default="reports", help="Folder to write the report files into")
_ARGS = _parser.parse_args()

FILE_PATH        = _ARGS.invoice
...
def sales_channel_series(category: pd.Series, branch: pd.Series) -> pd.Series:
    ...
    Vectorized over the whole column (no DataFrame.apply(axis=1)): applying
    row-by-row forces pandas to materialize the entire frame as an object
    array first, which is what was blowing up memory on the full dataset.
    """
    branch = branch.astype(str).str.strip().str.upper()
    is_project = (category == "Sales Project") | (branch == "PROJECT")
    is_agen = (
        (category == "Sales Agen")
        | (branch == "AGEN")
        | branch.str.startswith("DIST ")
        | branch.isin({"DIST KALIMANTAN", "DIST SUMATERA"})
    )
    return pd.Series(
        np.select([is_project, is_agen], ["Project", "Agen"], default="Retail"),
...
def assign_sku_series(df: pd.DataFrame) -> pd.Series:
    """Vectorized version of the per-row PIPAMAS_BRANDS sku rule.

    Avoids DataFrame.apply(axis=1) for the same memory reason as
    sales_channel_series.
    """
    idx        = df.index
    brand      = df["Brand Name"].astype(str) if "Brand Name" in df.columns else pd.Series("Unknown", index=idx)
    besides    = df["Product Category"] if "Product Category" in df.columns else pd.Series(np.nan, index=idx)
    sku_bogor  = df["grup sku bogor"] if "grup sku bogor" in df.columns else pd.Series(np.nan, index=idx)
    sku_def    = df["grup sku"] if "grup sku" in df.columns else pd.Series(np.nan, index=idx)
    branch_col = df["Branch"] if "Branch" in df.columns else pd.Series("", index=idx)

    is_pip = brand.str.strip().str.lower().isin(PIPAMAS_BRANDS)
    pip_result = sku_bogor.where(branch_col == "Bogor", sku_def)
    besides_valid = besides.notna() & (besides.astype(str).str.strip() != "")
    non_pip_result = besides.where(besides_valid, brand)

    result = pip_result.where(is_pip, non_pip_result)
    result = result.where(result.notna() & (result.astype(str).str.strip() != ""), brand)
    return result.astype(str).str.strip().str.lower()
...
def load_data() -> pd.DataFrame:
    ...
    df["Sales_Channel"]    = sales_channel_series(df["Sales_Category"], df["Branch"])
...
def page_11_directives(k: dict, v: dict) -> str:
    ...
    content = f"""
      ...
      <div class="grid-3">
        ...
        {insight('Target Control Focus', execution_text, 'red')}
      </div>
    """
...
def main() -> str:
    ...
    print("\n[7/7] Selesai.")
    print(f"[OK] Report tersimpan di: {pdf}")
    return pdf
