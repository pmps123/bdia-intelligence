...
# Data Transform Marketing pipeline.
...
auth = None  # kept for compatibility; credentials are loaded explicitly below
...
# KONFIGURASI
# ═══════════════════════════════════════════════════════════
_parser = argparse.ArgumentParser(description="Data Transform Marketing")
_parser.add_argument("--so", required=True, help="Sales Order export .xlsx")
...
INPUT_FILE_PATH      = _ARGS.so

# BigQuery target
BQ_PROJECT_ID = os.environ.get("BQ_PROJECT_ID", "pipamas-v3")
BQ_DATASET_ID = os.environ.get("BQ_DATASET_ID", "data")
...
PRODUCT_PIPAMAS_PATH   = _ARGS.target
PRODUCT_LAINNYA_PATH   = _ARGS.brand
...
def main():
    ...
    if OUTPUT_TO_BIGQUERY:
        ...
        client = bigquery.Client(project=BQ_PROJECT_ID, location=BQ_LOCATION, credentials=BQ_CREDENTIALS)
